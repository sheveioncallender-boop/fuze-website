(() => {
  ...
  const callOdoo = async (endpoint, params) => {
    ...
    if (envelope.error) {
      throw new Error(envelope.error.data?.message || envelope.error.message || 'The kitchen action could not be saved.');
    }
    if (!envelope.result) throw new Error('Odoo returned an empty kitchen response.');
  ...
  const postAction = async (endpoint, payload) => {
    const result = await callOdoo(endpoint, { payload: { ...payload, displayId: live.displayId } });
    if (!result.ok) throw new Error(result.message || 'The kitchen action could not be saved.');
    return result;
  };
