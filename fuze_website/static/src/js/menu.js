(() => {
  ...
  const checkoutDestinationPhone = document.getElementById('checkoutDestinationPhone');
  const checkoutSubtotal = document.getElementById('checkoutSubtotal');
  const checkoutAddress = document.getElementById('checkoutAddress');
  const customerEmail = whatsappOrderForm.elements.customerEmail;
  const checkoutSubmitStatus = document.getElementById('checkoutSubmitStatus');
  const data = window.FUZE_MENU;

  const LOCATION_KEY = 'fuze-order-location';
  const CART_KEY = 'fuze-order-cart';
  const ORDER_EMAIL = 'freshlyfuzed@gmail.com';
  const EMAIL_ENDPOINT = '/fuze/order/email';
  const locations = {
    east: { name: 'East Gates Mall', phone: '868-292-FUZE', whatsapp: '18682923893' },
    bagshot: { name: 'Bagshot BoxPark', phone: '868-336-FUZE', whatsapp: '18683363893' }
  };
  ...
  const itemId = (category, index) => `${category.id}-${index}`;
  ...
  const findItem = (identifier) => {
    const category = data.find((group) => identifier.startsWith(`${group.id}-`));
    if (!category) return null;
    const index = Number(identifier.slice(category.id.length + 1));
    const item = category.items[index];
    return item ? { category, item, index } : null;
  };
  ...
  const closeCart = () => {
    ...
    window.setTimeout(() => {
      ...
      whatsappCheckout.classList.remove('is-visible');
      whatsappCheckout.hidden = true;
    }, 320);
  };
  ...
  const openWhatsappCheckout = () => {
    ...
    checkoutDestination.textContent = destination.name;
    checkoutDestinationPhone.textContent = destination.phone;
    checkoutSubtotal.textContent = `$${cart.reduce((sum, item) => sum + item.price * item.quantity, 0)}`;
    whatsappCheckout.hidden = false;
    requestAnimationFrame(() => whatsappCheckout.classList.add('is-visible'));
    window.setTimeout(() => whatsappOrderForm.elements.customerName.focus(), 180);
  };

  const closeWhatsappCheckout = () => {
    whatsappCheckout.classList.remove('is-visible');
    window.setTimeout(() => { whatsappCheckout.hidden = true; }, 280);
  };

  const buildOrderMessage = (formData) => {
    const destination = locations[selectedLocation];
    const lines = [
      '*FUZE WEBSITE ORDER*',
      `Location: ${destination.name}`,
      `Customer: ${formData.get('customerName').trim()}`,
      `Contact: ${formData.get('customerPhone').trim()}`,
      `Order type: ${formData.get('orderType')}`
    ];

    const email = formData.get('customerEmail').trim();
    if (email) lines.splice(4, 0, `Email: ${email}`);

    if (formData.get('orderType') === 'Delivery enquiry') {
      lines.push(`Delivery address: ${formData.get('deliveryAddress').trim()}`);
    }

    lines.push('', '*ITEMS*');
    cart.forEach((item, index) => {
      lines.push(`${index + 1}. ${item.quantity} × ${item.name} — $${item.price * item.quantity}`);
      item.selections.forEach((selection) => lines.push(`   • ${selection.label}: ${selection.value}`));
      if (item.note) lines.push(`   • Item note: ${item.note}`);
    });

    const orderNote = formData.get('orderNote').trim();
    const subtotal = cart.reduce((sum, item) => sum + item.price * item.quantity, 0);
    lines.push('', `*SUBTOTAL: $${subtotal} TTD*`);
    if (orderNote) lines.push(`Order note: ${orderNote}`);
    lines.push('', 'Please confirm item availability, the final total and pickup/delivery details. Thank you.');
    return lines.join('\n');
  };
  ...
  const openEmailFallback = (formData, message) => {
    const destination = locations[selectedLocation];
    const subject = `Fuze website order — ${destination.name} — ${formData.get('customerName').trim()}`;
    window.location.href = `mailto:${ORDER_EMAIL}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(message.replaceAll('*', ''))}`;
    checkoutSubmitStatus.classList.remove('is-error');
    checkoutSubmitStatus.textContent = `Your email app was opened with the order addressed to ${ORDER_EMAIL}.`;
    showToast('Your email app is ready. Tap send to submit the order.');
  };

  const submitEmailOrder = async (formData, message, submitButton) => {
    const isStandalonePreview = window.location.protocol === 'file:' || /Fuze-Menu-Premium-Preview\.html$/i.test(window.location.pathname);
    if (isStandalonePreview) {
      openEmailFallback(formData, message);
      return;
    }
    ...
    checkoutSubmitStatus.classList.remove('is-error');
    checkoutSubmitStatus.textContent = 'Sending your order securely…';
    ...
    try {
      ...
      if (!response.ok || !result.ok) throw new Error(result.message || 'The order could not be emailed.');
      checkoutSubmitStatus.textContent = result.message;
      showToast('Order emailed to the Fuze team for confirmation.');
    } catch (_error) {
      openEmailFallback(formData, message);
    } finally {
  ...
  categories.addEventListener('click', (event) => {
    ...
    history.replaceState(null, '', selectedCategory === 'all' ? '/menu#fullMenu' : `/menu#${selectedCategory}`);
  });
  ...
  document.getElementById('continueCheckout').addEventListener('click', openWhatsappCheckout);
  document.getElementById('checkoutBack').addEventListener('click', closeWhatsappCheckout);

  whatsappOrderForm.addEventListener('change', (event) => {
    if (event.target.name !== 'orderType') return;
    const needsAddress = event.target.value === 'Delivery enquiry';
    checkoutAddress.hidden = !needsAddress;
    checkoutAddress.querySelector('textarea').required = needsAddress;
  });

  whatsappOrderForm.addEventListener('submit', async (event) => {
    event.preventDefault();
    const submitMethod = event.submitter?.dataset.submitMethod || 'whatsapp';
    customerEmail.required = submitMethod === 'email';
    if (!whatsappOrderForm.reportValidity()) return;
    const destination = locations[selectedLocation];
    if (!destination) {
      showToast('Please choose a Fuze location.');
      return;
    }
    ...
    if (submitMethod === 'email') {
      await submitEmailOrder(formData, message, event.submitter);
      return;
    }

    const whatsappUrl = `https://wa.me/${destination.whatsapp}?text=${encodeURIComponent(message)}`;
    window.open(whatsappUrl, '_blank', 'noopener,noreferrer');
    showToast(`WhatsApp opened for ${destination.name}. Tap send to submit your order.`);
  });
