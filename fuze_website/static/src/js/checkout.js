(() => {
  ...
  const readOdooResponse = async (response) => {
    ...
    try {
      ...
      throw new Error(`Odoo could not process this order (HTTP ${response.status}). Please try again or contact Fuze.`);
    }
  };
  const callOdoo = async (endpoint, params) => {
    ...
    if (envelope.error) {
      throw new Error(envelope.error.data?.message || envelope.error.message || 'Odoo could not process this order.');
    }
    if (!envelope.result) throw new Error('Odoo returned an empty order response.');
  ...
  const submitOdooOrder = async () => {
    ...
    const result = await callOdoo('/fuze/api/order/create', { payload });
    if (!result.ok) throw new Error(result.message || 'Your order could not be created.');
