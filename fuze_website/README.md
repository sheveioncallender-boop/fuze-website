...
The main Fuze stylesheet is registered in Odoo's versioned frontend asset
bundle. Release query versions are also applied to direct page assets so a
Cloudpepper deployment cannot leave an older checkout or kitchen layout in the
browser cache after a module upgrade.

Checkout, contact and kitchen actions use standard Odoo HTTP form submissions
with session CSRF tokens. The JSON order data is enclosed in the form payload,
which prevents Odoo's JSON-RPC dispatcher from intercepting these HTTP routes.
...
Users with Odoo's **Settings / Administrator** permission automatically receive
full create, read, update and delete access to all Fuze stations and displays,
all Fuze configuration menus and the kitchen display. They do not need a
separate Fuze Kitchen group.
