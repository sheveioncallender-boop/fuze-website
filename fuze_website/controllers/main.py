...
class FuzeWebsite(http.Controller):
    ...
    def create_order(self, payload=None, **kwargs):
        if (request.httprequest.content_length or 0) > 250_000:
            return self._api_result(False, "This order is too large to submit.", status=413)
        if not isinstance(payload, dict):
            return self._api_result(False, "The order details could not be read.", status=400)
        if self._clean(payload.get("website"), 100):
            return self._api_result(True, "Your order was received.")

        now = int(time.time())
        last_submission = int(request.session.get("fuze_order_last_submission", 0) or 0)
        if last_submission and now - last_submission < 5:
            return self._api_result(False, "Please wait a few seconds before submitting again.", status=429)

        try:
            # Keep a failed partner/order attempt from leaving partial records
            # in the request transaction while still returning a useful API result.
            ...
            return self._api_result(False, str(error), status=422)
        except Exception as error:
            _logger.exception("Fuze website POS order creation failed")
            message = "The order could not be created right now. Please call Fuze."
            if request.env.user.has_group("base.group_system"):
                message = "%s Administrator detail: %s" % (message, str(error))
            return self._api_result(False, message, status=500)
    ...
    @http.route("/fuze/contact/submit", type="jsonrpc", auth="public", website=True, methods=["POST"])
    def submit_contact(self, payload=None, **kwargs):
        if (request.httprequest.content_length or 0) > 100_000:
            return self._api_result(False, "This message is too large.", status=413)
        if not isinstance(payload, dict):
            return self._api_result(False, "The message could not be read.", status=400)
        if self._clean(payload.get("website"), 100):
            return self._api_result(True, "Thank you. Your message was received.")
        ...
        if len(name) < 2 or not self._valid_email(email) or len(message) < 10:
            return self._api_result(False, "Enter your name, a valid email and a complete message.", status=422)
        ...
        if not self._valid_email(recipient):
            return self._api_result(False, "The Fuze inbox is not configured.", status=500)
        body = (
        ...
        try:
            ...
            return self._api_result(False, "Email delivery is not available right now.", status=500)
        return self._api_result(True, "Thanks — your message has been sent to Fuze.")
    ...
    @http.route("/fuze/kitchen/order/advance", type="jsonrpc", auth="user", methods=["POST"])
    def kitchen_advance(self, payload=None, **kwargs):
        self._check_kitchen_access()
        payload = payload if isinstance(payload, dict) else {}
        order = self._authorized_kitchen_order(payload.get("orderId"), payload.get("displayId"))
        order.fuze_advance_kitchen_stage()
        return self._api_result(True, "Order moved to the next stage.")
    ...
    def kitchen_confirm_store_payment(self, payload=None, **kwargs):
        self._check_kitchen_access()
        payload = payload if isinstance(payload, dict) else {}
        order = self._authorized_kitchen_order(payload.get("orderId"), payload.get("displayId"))
        try:
            ...
            return self._api_result(False, str(error), status=422)
        return self._api_result(True, "Card-at-store payment recorded in Odoo POS.")

    @http.route("/fuze/kitchen/order/cancel-line", type="jsonrpc", auth="user", methods=["POST"])
    def kitchen_cancel_line(self, payload=None, **kwargs):
        self._check_kitchen_access()
        payload = payload if isinstance(payload, dict) else {}
        order = self._authorized_kitchen_order(payload.get("orderId"), payload.get("displayId"))
        line = order.lines.filtered(lambda item: item.id == self._safe_int(payload.get("lineId")))
        if not line:
            return self._api_result(False, "Order item not found.", status=404)
        ...
        if reason not in {"Out of stock", "Customer requested", "Duplicate item", "Kitchen error"}:
            return self._api_result(False, "Choose a valid cancellation reason.", status=422)
        line.write(
        ...
        return self._api_result(True, "Item cancelled and retained in the order audit trail.")
    ...
    def _create_pos_order(self, values):
        ...
        for raw in values["items"]:
            ...
            amount_tax += totals["total_included"] - totals["total_excluded"]
            selections = []
        ...
        if values["fulfilment"] == "delivery" and config.fuze_delivery_fee:
            ...
            amount_total += delivery_totals["total_included"]
            amount_tax += delivery_totals["total_included"] - delivery_totals["total_excluded"]
            line_commands.append(
                (
                    0,
                    0,
    ...
    def _check_kitchen_access():
        is_kitchen_user = request.env.user.has_group("fuze_website.group_fuze_kitchen_user")
        is_administrator = request.env.user.has_group("base.group_system")
        if not (is_kitchen_user or is_administrator):
            raise AccessError("You do not have access to the Fuze kitchen display.")

    @staticmethod
    def _api_result(ok, message, status=200, **values):
        return {"ok": ok, "message": message, "status": status, **values}
